param(
    ...
    [String]$Wallet, 
    [Parameter(Mandatory=$false)]
    [String]$UserName = "Asremix", 
    [Parameter(Mandatory=$false)]
    [String]$WorkerName = "ZcodeMiner",
    [Parameter(Mandatory=$false)]
    [String]$RigName = "AsremixPC",
    [Parameter(Mandatory=$false)]
    [Int]$API_ID = 0, 
    [Parameter(Mandatory=$false)]
    [String]$API_Key = "", 
...
if ((Get-Command "Get-MpPreference" -ErrorAction SilentlyContinue) -and (Get-MpComputerStatus -ErrorAction SilentlyContinue) -and (Get-MpPreference).ExclusionPath -notcontains (Convert-Path .)) {
    Start-Process (@{desktop = "powershell"; core = "pwsh"}.$PSEdition) "-Command Import-Module '$env:Windir\System32\WindowsPowerShell\v1.0\Modules\Defender\Defender.psd1'; Add-MpPreference -ExclusionPath '$(Convert-Path .)'" -Verb runAs
        ...
        Write-Host "                     //          //                     //              \\     ||          \\    ||                 " -foregroundcolor "Green"
        Write-Host "                    //          //                     //                \\    ||           \\   ||                 " -foregroundcolor "Green"
        Write-Host "                   //          ||                     ||     AsReMix      ||   ||            ||  Kudaraidee group   " -foregroundcolor "Green"
        Write-Host "                  //            \\                     \\                //    ||           //   ||                 " -foregroundcolor "Green"
        Write-Host "                 //              \\                     \\              //     ||          //    ||                 " -foregroundcolor "Green"
        Write-Host "                //                \\                     \\            //      ||         //     ||                 " -foregroundcolor "Green" 
        Write-Host "               //                  \\                     \\          //       ||        //      ||                 " -foregroundcolor "Green"  
...
$WalletDonate = "37mJin1qs8UscGpLygbiPVHq9vbkJvgcqE"
$UserNameDonate = "ZcodeMinerClient"
$WorkerNameDonate = "ZcodeMinerClient"
$WalletBackup = $Wallet
...
while($true)
{
    ...
    try {
        Write-Host "Zcode Miner dumps then checks for updates from Coinbase..." -foregroundcolor "Yellow"
    ...
    Write-Host -Level Warn "Pin on Coinbase. "
    ...
    if (($BestMiners_Combo | Where-Object Profit -EQ $null | Measure-Object).Count -eq 0) {
        $MinerComparisons = 
        [PSCustomObject]@{"Miner" = "Zcode.. sniffs out!"}, 
		...
		$host.ui.RawUI.WindowTitle = "Zcode Miner V1.2 - Mining " + $MinerComparisons[1] 
